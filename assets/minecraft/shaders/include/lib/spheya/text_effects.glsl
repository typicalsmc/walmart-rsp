...
void offset(float x, float y) { 
    textData.uv += vec2(-x, y) / 256.0; 
    textData.shouldScale = true;
}

void keycode_padding(vec3 midtone, vec3 highlight, vec3 shadow) {
    textData.shouldScale = true;
    ...
    // Middle
    for(int x = 1; x <= 2; x++) {
        vec2 uv = textData.uv + vec2(x, 0) * texelSize;
        if(!uvBoundsCheck(uv, textData.uvMin, textData.uvMax)) {
            vec4 s = texture(Sampler0, uv);
            if(s.a >= 0.1) { 
                int yIndex = int(textData.uv.y * 256.0); 

                vec3 newColor = textData.color.rgb;
                if (yIndex == 0) {
                    newColor = midtone;
                } else if (yIndex >= 1 && yIndex <= 3) {
                    newColor = highlight;
                } else if (yIndex >= 4 && yIndex <= 6) {
                    newColor = midtone;
                } else if (yIndex == 7) {
                    newColor = shadow;
                }

                if(textData.isShadow) newColor *= 0.25;
                textData.backColor = vec4(newColor, 1.0);
                return;
            }
        }
    }
...
void apply_shaking_movement(float speed) {
    ...
    textData.uv += vec2(noiseX, noiseY) / speed;
}
...
vec3 rainbow_color(bool useShadow) {
    vec3 color = hsvToRgb(vec3(0.005 * (textData.position.x + textData.position.y) - GameTime * 300.0, 0.7, 1.0));
    if(useShadow) color *= 0.25;
...
bool applyTextEffects() { 
    ...
    #moj_import<lib/spheya/text_effects_config.glsl>
...
bool applySpheyaPack9() {
    ...
    vctfx_isShadow = mod(Position.z, 1.0) < 0.01 ? 1.0 : 0.0;
    ...
    // Display behind other text
    gl_Position.z += textData.zDepth;
